...
function obterdados(idSetor) {
    fetch(`/medidas/tempo-real/${idSetor}`)
        .then(resposta => {
            if (resposta.status == 200) {
                resposta.json().then(resposta => {
                    ...
                    alertar(resposta, idSetor);
                });
            } else {
                console.error(`Nenhum dado encontrado para o id ${idSetor} ou erro na API`);
            }
        })
